...
*/
   ...
   public function sendMailFromStudentDashboard($body,$subject,$attachment='')
   {
     ...
     { 
         ...
         {    
           $adminUserMail=$admin->email;  
        
                Mail::raw($body,  function($message) use($emailFrom,$name,$adminUserMail,$subject)
                {
                 $message->to($adminUserMail)->subject($subject)->from($emailFrom,$name);
                });
        ...
        foreach($adminUser as $admin)
         {    
           $adminUserMail=$admin->email; 
                ...
                Mail::raw($body,  function($message) use($emailFrom,$name,$adminUserMail,$subject,$attachment)
                {
                 $message->to($adminUserMail)->subject($subject)->from($emailFrom,$name)->attachment($attachment);
                });
         }
